...
func main() {
	...
	for tc > 0 {
		...
		arrive := readNNums(reader, n)
		leave := readNNums(reader, n)

		for i := 0; i < n; i++ {
			guests[i] = []int{arrive[i], leave[i]}
		}
...
func solve(guests [][]int) int {
	...
	for i := 0; i < n; i++ {
		arrive, leave := guests[i][0], guests[i][1]
		events = append(events, Event{arrive, 1})
		events = append(events, Event{leave, -1})
	}
