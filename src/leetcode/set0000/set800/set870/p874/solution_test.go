...
func runSample(t *testing.T, commands []int, obstacles [][]int, expect int) {
	res := robotSim(commands, obstacles)
	if res != expect {
		t.Errorf("Sample %v %v, expect %d, but got %d", commands, obstacles, expect, res)
	}
}

func TestSample1(t *testing.T) {
	commands := []int{4, -1, 3}
	runSample(t, commands, nil, 25)
}

func TestSample2(t *testing.T) {
	commands := []int{4, -1, 4, -2, 4}
	obstacles := [][]int{{2, 4}}
	runSample(t, commands, obstacles, 65)
}

func TestSample3(t *testing.T) {
	commands := []int{2, -1, 8, -1, 6}
	obstacles := [][]int{{1, 5}, {-5, -5}, {0, 4}, {-1, -1}, {4, 5}, {-5, -3}, {-2, 1}, {-2, -5}, {0, 5}, {0, -1}}
	expect := 80
	runSample(t, commands, obstacles, expect)
}
