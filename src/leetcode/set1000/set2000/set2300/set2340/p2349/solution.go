...
func Constructor() NumberContainers {
	...
	return NumberContainers{nums, arr}
}

func (this *NumberContainers) Change(index int, number int) {
	if this.arr[index] > 0 {
		this.nums[this.arr[index]] = Delete(this.nums[this.arr[index]], index)
		this.arr[index] = 0
	}

	root := this.nums[number]
	root = Insert(root, index)
	this.nums[number] = root
	this.arr[index] = number
}

func (this *NumberContainers) Find(number int) int {
	if root, ok := this.nums[number]; !ok || root == nil {
		return -1
	} else {
		ans := root.MinValue()
		return ans
