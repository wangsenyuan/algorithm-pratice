...
func solve(n int, edges [][]int, rounds []int) []string {
	...
	dfs = func(p int, u int) {
		...
		for i := g.nodes[u]; i > 0; i = g.next[i] {
			...
			if p != v {
				dfs(u, v)
				cnt_lose_children[u] += lose[v]
			}
		}

		if cnt_lose_children[u] > 0 {
			lose[u] = 0
		} else {
			lose[u] = 1
		}
