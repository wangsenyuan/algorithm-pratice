package main

import (
	"bufio"
	"bytes"
	"fmt"
	"os"
	"sort"
	"strconv"
	"strings"
)

func main() {
	reader := bufio.NewReader(os.Stdin)

	n := readNum(reader)
	records := make([]string, n)
	for i := 0; i < n; i++ {
		records[i] = readString(reader)
	}

	res := solve(records)

	var buf bytes.Buffer

	buf.WriteString(fmt.Sprintf("%d\n", len(res)))

	for i := 0; i < len(res); i++ {
		buf.WriteString(fmt.Sprintf("%s\n", res[i]))
	}
	fmt.Print(buf.String())
}

func readFloat64(bs []byte, pos int, r *float64) int {
	var first float64
	var second float64
	exp := 1.0
	var dot bool
	for pos < len(bs) && (bs[pos] == '.' || isDigit(bs[pos])) {
		if bs[pos] == '.' {
			dot = true
		} else if !dot {
			first = first*10 + float64(bs[pos]-'0')
		} else {
			second = second*10 + float64(bs[pos]-'0')
			exp *= 10
		}
		pos++
	}
	*r = first + second/exp
	//fmt.Fprintf(os.Stderr, "%.6f ", *r)
	return pos
}

func isDigit(b byte) bool {
	return b >= '0' && b <= '9'
}

func readUint64(bytes []byte, from int, val *uint64) int {
	i := from

	var tmp uint64
	for i < len(bytes) && bytes[i] >= '0' && bytes[i] <= '9' {
		tmp = tmp*10 + uint64(bytes[i]-'0')
		i++
	}
	*val = tmp

	return i
}

func readInt(bytes []byte, from int, val *int) int {
	i := from
	sign := 1
	if bytes[i] == '-' {
		sign = -1
		i++
	}
	tmp := 0
	for i < len(bytes) && bytes[i] >= '0' && bytes[i] <= '9' {
		tmp = tmp*10 + int(bytes[i]-'0')
		i++
	}
	*val = tmp * sign
	return i
}

func readString(reader *bufio.Reader) string {
	s, _ := reader.ReadString('\n')
	for i := 0; i < len(s); i++ {
		if s[i] == '\n' || s[i] == '\r' {
			return s[:i]
		}
	}
	return s
}

func readNum(reader *bufio.Reader) (a int) {
	bs, _ := reader.ReadBytes('\n')
	readInt(bs, 0, &a)
	return
}

func readTwoNums(reader *bufio.Reader) (a int, b int) {
	res := readNNums(reader, 2)
	a, b = res[0], res[1]
	return
}

func readThreeNums(reader *bufio.Reader) (a int, b int, c int) {
	res := readNNums(reader, 3)
	a, b, c = res[0], res[1], res[2]
	return
}

func readNNums(reader *bufio.Reader, n int) []int {
	res := make([]int, n)
	x := 0
	bs, _ := reader.ReadBytes('\n')
	for i := 0; i < n; i++ {
		for x < len(bs) && (bs[x] < '0' || bs[x] > '9') && bs[x] != '-' {
			x++
		}
		x = readInt(bs, x, &res[i])
	}
	return res
}

func solve(records []string) []string {
	bestScore := make(map[string]Player)

	for _, rec := range records {
		cur := ParsePlayer(rec)
		if w, ok := bestScore[cur.name]; !ok || w.score < cur.score {
			bestScore[cur.name] = cur
		}
	}

	var players []Player

	for _, w := range bestScore {
		players = append(players, w)
	}

	sort.Slice(players, func(i, j int) bool {
		return players[i].score < players[j].score
	})

	n := len(players)

	better := make([]int, n)
	for i := n - 2; i >= 0; i-- {
		if players[i].score == players[i+1].score {
			better[i] = better[i+1]
		} else {
			better[i] = n - i - 1
		}
	}

	var res []string

	for i := 0; i < n; i++ {
		x := better[i]

		cat := "noob"
		if x*2 <= n {
			cat = "random"
			if x*5 <= n {
				// x <= 0.2 * n
				cat = "average"
				if x*10 <= n {
					// x <= 0.1 * n
					cat = "hardcore"
					if x*100 <= n {
						cat = "pro"
					}
				}
			}
		}

		res = append(res, fmt.Sprintf("%s %s", players[i].name, cat))
	}

	sort.Strings(res)

	return res
}

type Player struct {
	name  string
	score int
}

func ParsePlayer(s string) Player {
	ss := strings.Split(s, " ")
	name := ss[0]
	score, _ := strconv.Atoi(ss[1])
	return Player{name, score}
}
